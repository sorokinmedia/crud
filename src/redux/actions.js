const actions = {
	...
	deleteModel: (id, url, modelName) => ({type: actions.DELETE_MODEL, payload: {id, url, modelName} }),
	restoreModel: (id, url, modelName) => ({type: actions.RESTORE_MODEL, payload: {id, url, modelName} }),
	createModel: (form, url, modelName) => ({type: actions.CREATE_MODEL, payload: {form, url, modelName} }),
	changeModel: (form, action, modelName) => ({type: actions.CHANGE_MODEL, payload: {form, action, modelName} }),

	setModelModalForm: (modalType, initialValues, action) => ({
		type: actions.SET_MODEL_MODAL_FORM,
		payload: {modalType, initialValues, action}
	}),
	...
	fetchCrudFilterValues: (modelName, filter, query) => ({
		...
		payload: {query, filter, modelName}
	}),
};
